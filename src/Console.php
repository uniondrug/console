...
LOGO;
    ...
    public function registerCommands()
    {
        ...
        // 2019/01/29
        // 扫描Command存储目录, 兼容PHAR部署
        $cmdPath = $this->app->appPath().'/Commands';
        if (is_dir($cmdPath)) {
            $cmdHandle = dir($cmdPath);
            while (false !== ($cmdEntry = $cmdHandle->read())) {
                if (preg_match("/^(\S+Command)\.php/", $cmdEntry, $m) > 0) {
                    $command = '\\App\\Commands\\'.$m[1];
